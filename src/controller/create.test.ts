...
describe('create vehicle controller', () => {
  ...
  test('create with invalid id', async () => {
    // Given (mise en place du test).
    const req = {
        body: {
            shortcode: 'aaaaaaaaa',
    ...
    // When (exécution de la méthode testée).
    try {await controller.handle(req as Request, resp as unknown as Response);}
    // Then (validation des résultats.
    catch (err){ 
        //expect(err).toThrow(AppError)
        const myErr = err as AppError;
    expect(myErr.toString()).toBe("[2], Message Invalid create vehicle request, Details: {\"violations\":[\"Shortcode must be only 4 characters long\"]}");
    }
